...
class MyService {
    ...
    async getTransactionLinesByAgent(agentID) {
        return await this.papiClient.get(`/transaction_lines?where=Transaction.Agent.InternalID=${agentID}`)
    }

    async getTransactionsByAgent(agentID) {
        return (await this.papiClient.get(`/transactions?where=Agent.InternalID=${agentID}`))
    }

    createQuest(quest: Quest) {
        quest.Key = uuid();
        this.papiClient.addons.data.uuid(this.client.AddonUUID).table("Quests").upsert(quest);
    }

    async calcQuestsProgress() {
        const currUserId = (<any>jwtDecode(this.client.OAuthAccessToken))["pepperi.id"];
        const quests = await this.papiClient.addons.data.uuid(this.client.AddonUUID).table("Quests").find();
    ...
    async calculate(quest: Quest, agentID) {
        ...
        if (quest.Type == 'Quantity' && quest.Object == 'ItemID') {
            ...
            transLines = transLines.filter(line => line["Item"]["Data"]["InternalID"] == quest.ItemID)
        ...
        if (quest.Type == 'Price' && quest.Object == 'ItemID') {
            var transLines = await this.getTransactionLinesByAgent(agentID)
            transLines = transLines.filter(line => line["Item"]["Data"]["InternalID"] == quest.ItemID)
            var totalPricesArray = transLines.map(line => line["TotalUnitsPriceAfterDiscount"])
